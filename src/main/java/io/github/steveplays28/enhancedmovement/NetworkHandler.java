...
public class NetworkHandler {
	...
	public static void registerReceivers() {
		ServerPlayNetworking.registerGlobalReceiver(DOUBLE_JUMP_PACKET_ID, (server, player, handler, buf, responseSender) -> server.execute(() -> EnhancedMovement.getInstance().performMidAirJump(player)));
		...
		ServerPlayNetworking.registerGlobalReceiver(DASH_PACKET_ID, (server, player, handler, buf, responseSender) -> {
			...
			server.execute(() -> player.setVelocity(player.getVelocity().add(offsetX, offsetY, offsetZ)));
		});
	}
